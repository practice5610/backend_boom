      ...
      for (const booking of bookings) {
        ...
        if (booking.status === BookingStatus.USED || booking.status === BookingStatus.CANCELLED) {
          ...
          if (isOffer(item)) {
            ...
            const config: Config | null = await this.configRepository.findOne(
              configFilter as Filter<Config>
            );
            ...
            const commissionRate: number = config.value as number;
            ...
            this.logger.info(`Tax over price is: ${taxResponse.data.tax.amount_to_collect}`);
  ...
  async newPurchase(order: Order, options?: Options): Promise<APIResponse<any>> {
    try {
      ...
      for (const group of order.orderGroups) {
        if (!group.selectedRate)
          return APIResponseFalseOutput(`This group ${group} must have selectedRate.`);
        const shippingOrderResponse = await this.shippingService.purchase(
          group.selectedRate.shippo_id,
          customerData.uid,
          undefined
        );
      ...
      for (const bundle of validBookingsWithShippo) {
        ...
        const getTaxesResponse: APIResponse<TaxForOrderRes> =
      ...
      const orderHistoric: Order = { ...order, transactions: dbCreatedTransaction } as Order;

      const createOrderResponse = await this.orderService.create(orderHistoric);
