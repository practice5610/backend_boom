...
export class TaxController {
  ...
  @post('/getTaxableProduct', POSTGetTaxableProductSpecification)
  async getTax(
    @requestBody()
    item: {
      id: string;
    ...
    const result: any = [];
    console.log('workiiii', item);
    console.log('result12', result);

    await Promise.all(
      item.map(async (data) => {
        const targetBookingResponse: APIResponse<Booking> = await this.bookingService
          .getBookingById(data.id)
          .then((booking) => {
            console.log('booking12', booking);
            return booking;
          })
          .catch((error) => {
        ...
        console.log('checkfor', targetBookingResponse);
        ...
        >(targetBooking.item.merchantUID, {
        ...
        if (
          !merchantProfileData.taxableNexus.some((nexus) => nexus.state === data.toAddress.state)
        ) {
  ...
  async setTaxableStates(
    @requestBody(PUTSetTaxableStatesRequestBody) nexus: Nexus[]
  ): Promise<void> {
