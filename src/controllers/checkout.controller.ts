...
export class CheckoutController {
  ...
  async create(@requestBody() bookings: Booking[]): Promise<Transaction | Response> {
    ...
    try {
      const result: PurchaseResult = await this.purchaseService.purchase(bookings);
  ...
  @authorize([RoleKey.Member, RoleKey.Admin, RoleKey.SuperAdmin])
  @post('/place-order', POSTPlaceOrderSpecification)
  async placeOrder(@requestBody(POSTPlaceOrderRequestBody) order: Order): Promise<Response> {
    try {
